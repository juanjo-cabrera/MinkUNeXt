...
# convert to ME the previous classes
def gem(x, p=torch.ones(1)*3, eps: float = 1e-6):
    return F.avg_pool2d(x.clamp(min=eps).pow(p), (x.size(-2), x.size(-1))).pow(1./p)

class torch_GeM(nn.Module):
...
class Flatten(nn.Module):
    ...
    def forward(self, x: ME.SparseTensor):
        return x.F.view(x.F.size(0), -1)
    
class MEL2Norm(nn.Module):
    def __init__(self, dim=1):
        super(MEL2Norm, self).__init__()
        self.dim = dim

    def forward(self, x: ME.SparseTensor):
        x_norm = F.normalize(x.F, p=2.0, dim=self.dim)
        out = ME.SparseTensor(x_norm, coordinate_map_key=x.coordinate_map_key,
                                coordinate_manager=x.coordinate_manager)
        return out
...
class MinkUNeXt2(ResNetBase):
    ...
    def network_initialization(self, in_channels, out_channels, D):
        ...
        self.agregation = nn.Sequential(
            MEL2Norm(),
            GeM(input_dim=self.PLANES[7]),
            # Flatten(),
