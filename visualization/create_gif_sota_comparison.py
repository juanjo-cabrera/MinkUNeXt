...
def construct_query_and_database_sets(base_path, runs_folder, folders, pointcloud_fols, filename, p, output_name):
    ...
    for folder in folders:
        ...
        results_dir = PARAMS.dataset_folder + '/SOTA_VISUAL_RESULTS/' + runs_folder + folder + '/' 
        ...
        plot_pcds_and_positions(df_results_minkunext, df_results_minkloc3dv2, df_results_casspr, df_database, dst_images_dir_minkunext, dst_images_dir_minkloc3dv2, dst_images_dir_casspr, results_dir, p)
...
def plot_pcds_and_positions(df_results_minkunext, df_results_minkloc3dv2, df_results_casspr, df_database, dst_images_dir_minkunext, dst_images_dir_minkloc3dv2, dst_images_dir_casspr, output_path, p):
    ...
    for index, row_minkunext in df_results_minkunext.iterrows():
        if i % k == 0:
            ...
            # Plot zoomed map focusing on the relevant points
            axes[5].scatter(df_database['easting']/1000, df_database['northing']/1000, color='blue', s=50, alpha=0.6, label="Database Positions")
            ...
            axes[5].scatter(row_casspr['retrieved_database_x']/1000, row_casspr['retrieved_database_y']/1000, 
                          color='brown', marker='^', s=150, label="CASSPR Retrieved")
            
            axes[5].set_title("● Database positions", fontsize=SUBTITLE_FONT_SIZE, color='blue')
